...
それはシステムにインストールされている全てのPerlモジュール文書--コアモジュールであるか、総合Perlアーカイブネットワーク（CPAN: Comprehensive Perl Archive Network）からインストールしたモジュールであるかを問いません--と、もちろん何千ページにも及ぶPerlの大量のコア文書も表示します。
